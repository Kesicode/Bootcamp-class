    ...
    >
      ...
      <div className="border-b border-black/[0.06] pb-8">
        ...
        <p className="text-black/40 mt-2 font-mono text-xs tracking-wider uppercase">
          ROLE_NODE: <span className="text-black font-bold">{user?.role?.toUpperCase() || "STUDENT"}</span> // SYSTEM_ACCESS: GRANTED
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="border border-black/[0.06] rounded-xl p-8 bg-[#F8F9FA] relative overflow-hidden">
            <div className="absolute top-3 right-4 font-mono text-[8px] text-black/10 pointer-events-none select-none">
              PROGRESS_MATRIX
            </div>
            ...
            {totalDays === 0 && (
              <p className="font-mono text-[10px] text-black/25 uppercase tracking-widest mt-6">
        ...
        <div className="border border-black/[0.06] rounded-xl p-6 bg-[#F8F9FA]">
          ...
          {leaderboard === undefined ? (
            ...
            <p className="font-mono text-[10px] text-black/20 uppercase tracking-widest">NO_DATA</p>
          ) : (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={`font-mono text-[10px] font-bold w-6 ${
                      ...
                      i === 1 ? "text-gray-400" :
                      i === 2 ? "text-amber-500" :
                      u._id === user?._id ? "text-white/50" : "text-black/20"
                    }`}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className={`font-mono text-xs ${u._id === user?._id ? "text-white font-bold" : "text-black/60"}`}>
                      {u.name || u.email?.split("@")[0] || "ANONYMOUS"}
                      {u._id === user?._id && " (you)"}
                    </span>
                  </div>
                  <span className={`font-mono text-[10px] font-bold ${u._id === user?._id ? "text-white/70" : "text-green-700"}`}>
                    {u.streakCount || 0}d
                  </span>
                </div>
              ))}
...
function ProgressBar({ label, pct, detail, delay, color }) {
  return (
    <div>
      ...
      <div className="h-[2px] w-full bg-black/5 rounded-full overflow-hidden mb-1">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: `${pct}%` }}
      ...
      <p className="font-mono text-[9px] text-black/25 uppercase tracking-wider">{detail}</p>
    </div>
  );
}
...
function StatCard({ label, value, unit, index }) {
  return (
    <motion.div
      ...
      className="border border-black/[0.06] rounded-xl p-6 bg-[#F8F9FA] hover:bg-white transition-colors relative overflow-hidden"
    >
      <div className="absolute top-2 right-3 font-mono text-[8px] text-black/10 select-none pointer-events-none">NODE</div>
